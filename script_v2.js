...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function smoothZoomOut() {
    // Store current positions as starting points for interpolation
    if (model) {
      zoomOutStartRotation.copy(model.rotation);
    }
    if (camera) {
  ...
  function panLeft() {
    ...
    const direction = xAxis.multiplyScalar(-panSpeed);
    smoothPan(direction);
  }
  function panRight() {
    const panSpeed = 0.1;
  ...
  function panUp() {
    ...
    const zAxis = new THREE.Vector3();
    camera.matrix.extractBasis(xAxis, yAxis, zAxis);
    const direction = yAxis.multiplyScalar(panSpeed);
    smoothPan(direction);
  }
  function panDown() {
    const panSpeed = 0.1;
  ...
  function smoothPan(direction) {
    ...
    isPanning = true;
    panProgress = 0;
    panStart.copy(controls.target);
    panTarget.copy(controls.target).add(direction);
  }
  ...
  function animate() {
    ...
    if (isZoomingOut) {
      ...
      if (zoomOutProgress <= 1) {
        ...
        if (model) {
          model.rotation.x =
            zoomOutStartRotation.x +
            (initialRotation.x - zoomOutStartRotation.x) * easeOut;
          model.rotation.y =
            zoomOutStartRotation.y +
            (initialRotation.y - zoomOutStartRotation.y) * easeOut;
          model.rotation.z =
            zoomOutStartRotation.z +
            (initialRotation.z - zoomOutStartRotation.z) * easeOut;
        }
    ...
    // Smooth panning
    if (isPanning) {
      panProgress += 0.1; // Panning speed
      if (panProgress <= 1) {
        const easeOut = 1 - Math.pow(1 - panProgress, 3);
        controls.target.lerpVectors(panStart, panTarget, easeOut);
      } else {
        isPanning = false;
...
document.addEventListener("keydown", function (event) {
  if (event.target.tagName === "INPUT" || event.target === modelContainer) return;
